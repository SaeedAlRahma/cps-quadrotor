    ...
    if isMoving and not dataFile.closed:
        ...
        if not count_period%WRITE_PERIOD:
            ...
            if isTargetReached(position, cmdPosition, twistLinear):
                ...
                if count_pos < len(PATH):
                    cmd = PATH[count_pos]
                    cmdPosition.x = position.x + (MOVE_BINDINGS[cmd][0]*DIST_TRAVEL)
                    cmdPosition.y = position.y + (MOVE_BINDINGS[cmd][1]*DIST_TRAVEL)
                    cmdPosition.z = position.z + (MOVE_BINDINGS[cmd][2]*DIST_TRAVEL)
                    publishPoseCommand()
                else:
                    isMoving = False
                count_pos += 1
